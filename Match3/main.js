{
    ...
    const cellSize = 50;
    const fieldSize = parseInt(prompt("Введите разммер поля", 12));
        ...
        select(cell) {
            if (this.hasHead) {
                if (this.head == cell) {
                    ...
                    //check if neighbour is swappable
    ...
    function clickHandler(event) {
        if (event.isTrusted) {
            gameStart = true;
            clickedCells = getClickedCell(event);

            selectionHandler.select(fieldArray[clickedCells[0]][clickedCells[1]]);
            drawCells();
        } else {
            alert("N0 B0TS!");
        }
    ...
    function getClickedCell(event) {
        cellX = Math.floor((event.pageX - 8) / cellSize);
        cellY = Math.floor((event.pageY - 8) / cellSize);
    ...
    function tick() {
        if (scoreMultipier < 1) {
            ...
            let lastGameVer = localStorage.getItem("GameVer");
